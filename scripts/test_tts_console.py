...
class TestSourceMap(unittest.TestCase):
    ...
    def test_a_compiled_global_line_maps_back_into_the_sources(self):
        ...
        needle = "function battleLogEncode()"
        source_line = None
        companion = LUA_DIR / "battleLog.ttslua"
        for n, text in enumerate(companion.read_text(encoding="utf-8").splitlines(), 1):
            if text.strip() == needle:
                source_line = n
                break
        self.assertIsNotNone(source_line, f"{needle} is gone from battleLog.ttslua")
        ...
        self.assertEqual(path, "TTSLUA/battleLog.ttslua")
    ...
    def test_a_line_that_is_not_itself_distinctive_still_maps(self):
        ...
        needle = "function battleLogOnSave()"
        compiled_line = None
        for n, text in enumerate(self.smap.compiled_global, 1):
            if text.strip() == needle:
                compiled_line = n
        ...
        self.assertEqual(path, "TTSLUA/battleLog.ttslua")
        src = (LUA_DIR / "battleLog.ttslua").read_text(encoding="utf-8").splitlines()
